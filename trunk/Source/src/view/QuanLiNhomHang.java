...
public class QuanLiNhomHang extends JInternalFrame implements ActionListener {
	 ...
	 * @throws SQLException 
	 */
	public QuanLiNhomHang() throws SQLException {
		lblThongBao = new JLabel();
		lblThongBao.setText("M\u00E3 nh\u00F3m h\u00E0ng");
		...
		JLabel lblTniTc = new JLabel("T\u00EAn nh\u00F3m h\u00E0ng: ");
		lblTniTc.setAlignmentX(Component.CENTER_ALIGNMENT);
		...
		textField_GhiChuDT.setColumns(50);
		...
		model = (new DefaultTableModel(
				new Object[][] {
					...
					new String[] {
						"STT", "M\u00E3 nh\u00F3m h\u00E0ng", "T\u00EAn nh\u00F3m h\u00E0ng", "Ghi ch\u00FA"
					}
				) {
...
//		updateb = 0;
	...
	{
		QLNhomHangProcess qlNhomHangPro = new QLNhomHangProcess();
		Vector<String> data = new Vector<String>();
		data = qlNhomHangPro.getSuggestData(textfJSuggestField.getText());
		System.out.println("---------------------------------------");
	...
	public void export(){
		...
		QLNhomHangProcess ql = new QLNhomHangProcess();
		Vector<Vector<String>> duLieu = new Vector<Vector<String>>();
		Vector<NhomHang> NhomHang;
		try {
			NhomHang = ql.getListNhomHang(1);
			for (int k = 0; k < NhomHang.size(); k++) {
				Vector<String> phanTu = new Vector<String>();
				phanTu.add(NhomHang.get(k).getsTenNhomHang());
				System.out.println(NhomHang.get(k).getPK_sNhomHangID().toString());
				phanTu.add(NhomHang.get(k).getPK_sNhomHangID().toString());
				phanTu.add(NhomHang.get(k).getsSoDienThoai());
				phanTu.add(NhomHang.get(k).getsDiaChi());
				phanTu.add(NhomHang.get(k).getsNguoiLienHe());
				phanTu.add(NhomHang.get(k).getsGhiChu());
				duLieu.add(phanTu);
			}
			test.write(8, 1, duLieu, "Temp.xls");
			System.out.println(NhomHang.get(0).getClass().getCanonicalName());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void openText() {
			...
			textField_MaNH.setEditable(true);
	...
	public void closeText() {
			...
			textField_GhiChuDT.setEditable(false);
			textField_MaNH.setEditable(false);
	...
	public void loaddata() {
		try {
			...
			Integer soDT;
		QLNhomHangProcess dtac = new QLNhomHangProcess();
		if (findb == 0) {
			soDT = dtac.getsoNhomHang();
		...
		Vector<NhomHang> dsNhomHang;
		if (findb == 1) {
			dsNhomHang = dtac.getListSearched(soTrang,textfJSuggestField.getText());
		} else {
			dsNhomHang = dtac.getListNhomHang(soTrang);
		}
		sTT = (soTrang-1)*10+1;
		System.out.println("So thu tu: "+sTT);
		System.out.println("So trang: "+soTrang);
		System.out.println("So doi tac trong trang: "+dsNhomHang.size());
		for (int i = 0; i < dsNhomHang.size(); i++) {
			Object[] oPerson = {sTT,
								dsNhomHang.get(i).getPK_sNhomHangID(),
								dsNhomHang.get(i).getsTenNhomHang(),
								dsNhomHang.get(i).getsNguoiLienHe(),
								dsNhomHang.get(i).getsDiaChi(),
								dsNhomHang.get(i).getsSoDienThoai(),
								dsNhomHang.get(i).getsGhiChu()
								};
			sTT++;
	...
	public void loadLuoiTable(){
		...
		textField_MaNH.setText(((String)table.getModel().getValueAt(row, 1)));
	...
	public void insert()
	{
		try {
		QLNhomHangProcess dtac = new QLNhomHangProcess();
		if (dtac.checkPK(textField_MaNH.getText()) == true && insertb == 1 && loi == 0 && loiDiaChi == 0 && loiGhiChu == 0 && loiMaDT ==0 && loiNLH==0 && loiSDT==0 && loiTenDT ==0) {
...
//				CheckTen checkTen = new CheckTen();
				...
				NhomHang.setiTrangThai(1);
				NhomHang.setPK_sNhomHangID(textField_MaNH.getText());
				NhomHang.setsDiaChi(textField_DiaChiDT.getText());
				NhomHang.setsGhiChu(textField_GhiChuDT.getText());
				NhomHang.setsNguoiLienHe(a.xuLiTen(textField_NLH.getText()));
				NhomHang.setsSoDienThoai(textField_SoDT.getText());
				NhomHang.setsTenNhomHang(a.xuLiTen(textField_TenDT.getText()));
				NhomHang.setsTenNhomHangEng(TiengVietToEg.convert(textField_TenDT.getText()));
				
				Integer soDT = dtac.getsoNhomHang();
				boolean kiemTra = dtac.ThemNhomHang(NhomHang);
...
//tao themmnut xoa nhieu nguoi va tao mang cong don cac Ma muon xoa	
	public void delete(){
		...
		if (deleteb == 1) {
			...
			try {
				if (dtac.checkPK(textField_MaNH.getText())) {
					// khoa chinh ko trung
					new ThongBao(lblThongBao, Color.RED,"Kh\u00F4ng t\u1ED3n t\u1EA1i \u0111\u1ED1i t\u00E1c mu\u1ED1n x\u00F3a !");
				} else {
					
					if (JOptionPane.showConfirmDialog(new JFrame(),
					        "Do you want to delete "+textField_MaNH.getText()+" ?", "Delete",
					        JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
					      
					 {
						dtac.XoaNhomHang((textField_MaNH.getText()));
...
//					CheckTen checkTen = new CheckTen();
					...
					Integer soDT = dtac.getsoNhomHang();
					boolean kiemTra = dtac.SuaNhomHang(NhomHang,(((String)table.getModel().getValueAt(row, 1))));
					System.out.println(kiemTra);
					if (kiemTra==true) {
...
//		System.out.println("------------------------------------------------------------------");
		if (updateb ==1) {
			if (textField_MaNH.getText().equals((String)table.getModel().getValueAt(table.getSelectedRow(), 1))) {
				if ((CheckString.OnlytextAndNumber(textField_MaNH.getText(), 6, 1, lblThongBao,"M\u00E3 \u0111\u1ED1i t\u00E1c: "))==false) {
					loiMaDT = 1;
					} else {
					loiMaDT = 0;
				...
				if (dtac.checkPK(textField_MaNH.getText())==false){
					loiMaDT = 1; 
