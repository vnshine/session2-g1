...
public class CheckString {
	...
	public boolean OnlytextAndNumber(String str,Integer max,Integer min) {
		Integer loi = 0;
		str = str.trim();//xoa khoang trang dau duoi
		if (str.length() < min) loi = 1;//kiem tra do dai ten toi thieu
		if (str.length() > max) loi = 1;//kiem tra do dai ten toi da
		if (str.matches("[A-z0-9]+")) {}else loi = 1;
		if (loi == 1) {
			return false;
		} else {
			return true;
...
//			result[i] = arrChar[i];
	...
	public static void main(String[] args) {
		...
		System.out.println(a.OnlytextAndNumber("a66946", 6, 1));
	}
}
